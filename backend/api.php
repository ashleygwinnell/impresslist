...
function api_checkRequiredGETFieldsWithTypes($fields, &$result) {
	return api_checkRequiredREQUESTFieldsWithTypes($fields, $result, $_GET);
}
function api_checkRequiredREQUESTFieldsWithTypes($fields, &$result, &$data) {
	for($i = 0; $i < count($fields); $i++) {
		if (!isset($data[$fields[$i]['name']])) {
			...
			if ($type == 'email') {
				...
				$temp = $data[$fields[$i]['name']]; // str_replace("%20", " ", $data[$fields[$i]['name']]);
				...
				$http = substr($data[$fields[$i]['name']], 0, 7);
				$https = substr($data[$fields[$i]['name']], 0, 8);
				...
				//$temp = strip_tags($data[$fields[$i]['name']]);
				//return true;
	...
	if (!in_array($endpoint, $endpoints)) {
		...
		else if ($endpoint == "/search/")
		{
			...
			if (!isset($_GET['q'])) {
				...
				$stmt = $db->prepare("select * from person
										...
										or surnames like :search
										or email like :search
										or notes like :search
									;");
				$stmt->bindValue(":search", $searchSql, Database::VARTYPE_STRING);
				$people = $stmt->query();
				...
				// personPublications
				$stmt = $db->prepare("select * from person_publication
		...
		else if ($endpoint == "/auth/check/")
		{
			...
			if ( $_SESSION['user'] ) {
				...
				$stmt->bindValue(":id", $_SESSION['user'], Database::VARTYPE_STRING);
				$results = $stmt->query();

				$result->success = true;
				$result->user = api_maskUser( $results[0] );;
			} else {
				$result->success = false;
		...
		else if ($endpoint == "/auth/login/")
		{
			...
			$data = api_getRequestData();

			$error = api_checkRequiredREQUESTFieldsWithTypes($required_fields, $result, $data);
			if (!$error)
			{
				$email = $data['email'];
...
//			header("Location: /");
		...
		else if ($endpoint == "/person/list/")
		{
			...
			$people = $db->query("SELECT person.*,
										user.forename as lastcontactedby_forename,
										user.surname as lastcontactedby_surname,
										user.color as lastcontactedby_color,
										max(e.utime) as lastcontacted_utime,
										e2.id as lastcontactedby_me
									FROM person
										LEFT JOIN user on person.lastcontactedby = user.id
										LEFT JOIN email e on person.id = e.person_id
										LEFT JOIN email e2 on person.id = e.person_id and e.user_id = " . $_SESSION['user'] . "
									WHERE removed = 0
									GROUP BY person.id
									ORDER BY person.id ; ");
			//$num_people = count(array_values($people));
			//usort($people, "sortByName");
			//usort($people, "sortById");

			foreach ($people as &$person) {
				$person['notes'] = utf8_encode($person['notes']);
			}
